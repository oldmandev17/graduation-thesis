      ...
      <td>
        ...
        >
          <MenuItem
            ...
            onClick={() => {
              navigate(`/user/${user?.id}/buyer-orders/${order?._id}`)
              handleCloseAction()
            }}
          >
            View
          ...
          <MenuItem
            className='!pr-20 !py-2'
            onClick={() => {
              navigate(`/user/${user?.id}/messages?to=${order.gig && order.gig.createdBy && order.gig.createdBy.id}`)
              handleCloseAction()
            }}
          >
            Send Message
...
function ManageBuyerOrderPage() {
  ...
  const getAllOrderByUsers = useCallback(async () => {
    await getAllOrderByUser(accessToken, status, keywordDebounce, sortBy, orderBy, 'buyer')
      .then((response) => {
  ...
  return (
    <>
      <Helmet>
        <title>Manage Orders | Freelancer</title>
      </Helmet>
      <div className='flex flex-col h-screen gap-5 py-10 px-28'>
        <div className='flex flex-row justify-between'>
          <span className='text-4xl font-semibold text-gray-600'>Manage Buyer Orders</span>
          <input
            type='text'
...
export default ManageBuyerOrderPage
